 ...
 *
 */
public class DCWebUrlFromNombreEmpresa_Investing
{

	/**
	 * Logger
	 */
	private final static Logger LOGGER = LoggerFactory.getLogger(DCWebUrlFromNombreEmpresa_Investing.class);
	 ...
	 * 
	 */
	private final static String URL_INICIAL = "https://es.investing.com/search/?q=";
		...
		for (URLGroup empresasGroup : LISTA_EMPRESAS_GROUP)
		{
			...
			while (empresaIdx < listaEmpresas.size())
			{
				...
				try
				{
					...
					String busquedaURL = URL_INICIAL + URLEncoder.encode(nombreEmpresa, "UTF-8") + "&nocache=" + System.currentTimeMillis();
					LOGGER.info("Cargando URL de búsqueda [" + busquedaURL + "]");
					driver.get(busquedaURL);
					new WebDriverWait(driver, 10).until(ExpectedConditions.presenceOfElementLocated(By.className("js-inner-all-results-quotes-wrapper")));
					WebElement tablaResultados = driver.findElement(By.className("js-inner-all-results-quotes-wrapper"));
					List<WebElement> listaLinks = tablaResultados.findElements(By.tagName("a"));
					String resultHrefElemento = null;
					for (WebElement link : listaLinks)
					{
						String linkHref = link.getAttribute("href");
						if (linkHref != null && linkHref.indexOf("es.investing.com/equities") != -1)
						{
					...
					LOGGER.info(" --- URL Encontrada [" + downloadFolder + "] [" + nombreEmpresa + "] [" + resultHrefElemento + "]");
					empresaIdx++;
					errorRetry = 0;
					int msEspera = 1000;
				...
				catch (Exception e)
				{
					...
					{
						LOGGER.info(" --- URL NO Encontrada [" + downloadFolder + "] [" + nombreEmpresa + "] [-]");
						empresaIdx++;
						errorRetry = 0;
					}
					int msEspera = 5000;
