...
public class Investing
{
	 ...
	 * Logger
	 */
	private final static Logger LOGGER = LoggerFactory.getLogger(Investing.class);
			...
			try
			{
				...
				while (hrefsIdx < listaHrefs.size())
				{
					try
					{
						...
						LOGGER.info("Escribiendo fichero");
						String fileName = DOWNLOAD_PATH + "\\" + URLEncoder.encode(hrefElemento, CHARSET) + ".txt";
						FileUtils.writeLines(new File(fileName), CHARSET, lineasFichero);
